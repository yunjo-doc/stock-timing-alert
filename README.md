...
`/admin/members` 에서 로그인하면 전체 회원의 이름/전화번호/이메일/가입일/
현재 구독 플랜/다음 결제일/관심종목 목록과, 각 종목별로 가장 최근에 매수
신호가 떴던 날짜(최근 매수 제안일)와 매도 신호가 떴던 날짜(최근 매도
제안일)를 한눈에 볼 수 있습니다. 관리자는 회원의 구독 플랜을 결제 없이
직접 올리거나 내릴 수 있고, 계정을 정지/해제할 수도 있습니다.

**관리자 로그인은 일반 회원가입과 같은 이메일/비밀번호를 사용합니다.**
1. `/signup` 에서 평소처럼 회원가입
2. `/admin/login` 에서 그 이메일/비밀번호로 로그인 시도
3. 아직 관리자 권한이 없는 계정이면 "관리자 코드" 칸에 Render 환경변수
   `ADMIN_TOKEN` 값을 한 번만 입력 — 이때 해당 계정이 관리자로 승격됩니다.
4. 이후로는 관리자 코드 없이 이메일/비밀번호만으로 `/admin/login`이 됩니다.

`ADMIN_TOKEN`은 이제 "관리자 최초 승격 코드" 겸 `/run-now`를 외부에서
(로그인 없이) 호출할 때 쓰는 API 토큰 용도로만 쓰입니다.
...
## 데이터 영구 보존 (Persistent Disk) — 반드시 설정하세요

**중요:** Render는 코드가 배포된 디스크를 **재배포할 때마다 완전히 새로 만듭니다.**
`DATA_DIR`(기본값: 프로젝트 폴더 안의 `data/`)를 Persistent Disk로 지정해두지
않으면, GitHub에 새 커밋을 올려 자동 재배포될 때마다 회원가입 데이터/관심종목/
구독/분석이력이 담긴 SQLite 파일(`data/app.db`)이 통째로 초기화됩니다.
유료 플랜으로 슬립을 없애도 이 문제는 별개로 계속 발생합니다.

**설정 방법 (Render 대시보드, 기존 서비스에 한 번만 하면 됩니다):**
1. Render 대시보드 → `stock-timing-alert` 서비스 → **Settings → Disk**
2. **Add Disk** 클릭
   - Name: 아무 이름 (예: `alphatiming-data`)
   - Mount Path: `/var/data`
   - Size: 1GB (충분함)
3. **Environment** 탭에서 환경변수 추가:
   - `DATA_DIR` = `/var/data`
4. 저장하면 서비스가 재배포됩니다 (이번 재배포 이후부터는 데이터가 유지됩니다.
   지금까지 초기화됐던 회원 데이터는 복구되지 않으니, 설정 후 다시 가입해주세요.)

이 저장소를 Render Blueprint(`render.yaml`)로 새로 만드는 경우에는 디스크 설정이
`render.yaml`에 이미 포함되어 있어 자동으로 적용됩니다.
...
- **네이버 금융 크롤링은 비공식**입니다. 페이지 구조 변경 시 파싱이 깨질 수
  ...
  제공업체 이용을 권장합니다.
- Render에 Persistent Disk를 연결하지 않으면 (무료/유료 플랜 관계없이) **재배포할
  때마다** SQLite 파일이 초기화됩니다. 위 "데이터 영구 보존" 섹션대로 Persistent
  Disk를 꼭 연결하세요. 그래도 더 장기적인 이력 보존이 필요하면 외부 DB
  (Supabase 등) 연동을 고려하세요.
